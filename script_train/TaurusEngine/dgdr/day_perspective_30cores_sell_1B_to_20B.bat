...
for /L %%L in (1,1,20) do (
    ...
    for %%D in (0.050 0.100 0.150 0.200 0.250 0.300) do (
        ...
        for %%P in (1.001 1.005 1.010 1.0125 1.015) do (
            ...
            start "DGDR_OPTUNA_L%%L" cmd /C "call conda activate PY312_HT && cd ..\..\..\src\optimizers\dgdr && python .\realtime_and_backtest_hyperparameter_search_optuna.py --dataset-id day --lookahead-bars %%L --method final_close --min-signal-density %%D --put-strike-pct 1. --call-strike-pct %%P --n-trials 99999 --timeout 3600 --train-ratio 0.90 --signal-type sell --output-dir day_perspective --optuna-storage sqlite:///day_perspective\\day_day_sell_!STRK_SUF!_dens!DENS_SUF!_lookahead%%L.db --optuna-study-name doesnotmatter"
        )
    )
