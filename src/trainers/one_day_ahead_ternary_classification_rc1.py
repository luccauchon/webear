...
from datasets import TripleIndicesLookAheadTernaryClassificationDataset
from models import LSTMClassification, LSTMMetaClassification
from utils import all_dicts_equal, namespace_to_dict, dict_to_namespace, get_stub_dir, get_df_SPY_and_VIX, generate_indices_with_cutoff_day, calculate_multiclass_classification_metrics, generate_indices_with_multiple_cutoff_day, generate_indices_basic_style, previous_weekday, next_weekday
...
def train(configuration):
    ...
    frequency_of_noise  = 0.75
    power_of_noise      = 0.01


    tav_dates = configuration.get("tav_dates", ["2022-01-01", "2025-03-15"])
    mes_dates = configuration.get("mes_dates", ["2025-03-16", "2025-03-30"])
    ...
    y_seq_length = 1
    pct_sideways = configuration.get("pct_sideways", 0.5)  # in %
    assert y_seq_length == 1

    run_id = configuration.get("run_id", 123)
    version = configuration.get("version", "rc1")
    output_dir = os.path.join(get_stub_dir(), f"{run_id}", f"trainer__one_day_ahead_ternary_classification__{version}")
    ...
    # Description
    ###########################################################################
    #print("\u2190 Left Arrow") print("\u2192 Right Arrow") print("\u2193 Down Arrow") print("\u2191 Up Arrow")
    logger.info("The goal is to predict the close value of next day (\u2191, \u2193 or \u2190\u2192) based on the preceding P days")
    ...
    lr_decay_iters = configuration.get("lr_decay_iters", 50001)
    max_iters      = configuration.get("max_iters", 50001)
    min_lr         = configuration.get("min_lr", 1e-5)
    stats_check    = True
    warmup_iters   = 0
    weight_decay   = 0.1
    ...
    train_dataset    = TripleIndicesLookAheadTernaryClassificationDataset(_df=train_df, _feature_cols=x_cols, _target_col=y_cols, _device=device, _x_cols_to_norm=x_cols_to_norm,
                                                                          _indices=train_indices, _mode='train', _data_augmentation=data_augmentation, _pct_sideways=pct_sideways,
                                                                          _frequency_of_noise=frequency_of_noise, _power_of_noise=power_of_noise, _num_classes=model_args['nb_classes'])
    test_dataset     = TripleIndicesLookAheadTernaryClassificationDataset(_df=test_df, _feature_cols=x_cols, _target_col=y_cols, _device=device, _x_cols_to_norm=x_cols_to_norm,
                                                                          _indices=test_indices, _mode='test', _data_augmentation=False, _pct_sideways=pct_sideways, _num_classes=model_args['nb_classes'])
        ...
        for desc, a_dataloader in zip(["train", "test"], [train_dataloader, test_dataloader]):
            ...
            for _x, _y, _x_data_norm in a_dataloader:
                for label in _y.unique():
                    class_counts[label.item()] = class_counts.get(label.item(), 0) + torch.count_nonzero(_y == label.item()).item()
                if desc == 'test':
                    ground_truth_sequence.extend(_y.cpu())
            ground_truth_sequence = [uu.item() for uu in ground_truth_sequence]
            total = sum(class_counts.values())
            _tmp_str = f"In {desc} dataset --> "
            for label, count in class_counts.items():
                _tmp_str += f"  {label}: {(count / total) * 100:.2f}%"
            logger.info(_tmp_str)
            if desc == 'test':
                logger.info(f"Test sequence: {ground_truth_sequence}")
    ###########################################################################
    # Model preparation
    ###########################################################################
    model = LSTMClassification(num_input_features=model_args['num_input_features'], binary_classification=False, nb_classes=model_args['nb_classes'],
                               hidden_size=model_args['hidden_size'], num_layers=model_args['num_layers'], seq_length=model_args['seq_length'],
                               bidirectional=model_args['bidirectional'], device=model_args['device'], dropout=model_args['dropout'])
    ...
    loss_function = torch.nn.CrossEntropyLoss(reduction='sum').to(device)  # mean-squared error for regression
    ...
    while iter_num < max_iters:
        ...
        if iter_num % eval_interval == 0 or 1 == iter_num or iter_num == max_iters - 1:
            ...
            for batch_idx, (x_test, y_test, _) in enumerate(test_dataloader):
                y_test = F.one_hot(y_test.type(torch.int64), num_classes=model_args['nb_classes'])
                ...
                test_accuracy.append(calculate_multiclass_classification_metrics(y_true=torch.argmax(y_test, dim=1), y_pred=test_logits, num_classes=model_args['nb_classes'])['accuracy'])
                # strp = f"{torch.argmax(y_test, dim=1)}, {torch.argmax(test_logits, dim=1)}"
        ...
        # Compute accuracy
        training_accuracy = calculate_multiclass_classification_metrics(y_true=_y, y_pred=train_logits, num_classes=model_args['nb_classes'])['accuracy']
    ...
    results = {'running__train_losses': running__train_losses, 'running__train_accuracy': running__train_accuracy.item(),
               ...
               'output_dir': output_dir,'data_augmentation': data_augmentation,
