...
def entry():
    ...
    experience_str_desc = f"{seuil_pos}" + f"__{fenetre_analysee}".replace('datetime.time', '').replace('(', '').replace(')', '').replace(',', '').replace(' ', '_')
    ...
    # ---------------------------------------------------------
    feature_cols = [
            'pct_above_vwap_morning', 'last_vwap_slope', 'last_vwap_deriv',
    ...
    df_sorted_results.to_csv(f'model_ranking_cv_all_combinations__{experience_str_desc}.csv',index=False)
