...
def entry():
    ...
    print(f"Construction du dataset dabs {output_filename_for_dataset}...")
    configuration = Namespace(ticker="^GSPC", dataset_id="day", step_back_range=260, look_ahead=1, epsilon=0.01, target="POS_SEQ", convert_price_level_with_baseline="fraction",
                              ...
                              model_overrides="{}", save_dataset_to_file_and_exit=output_filename_for_dataset)
    ...
    if 0 == predicted_classes:
        print(f"There is {prediction_probabilities[predicted_classes]*100:.0f}% chance of closing price be below {upper_close_value:.0f} on {next_day(date_of_data_point_x_used)}")
    elif 1 == predicted_classes:
        print(f"There is {prediction_probabilities[predicted_classes]*100:.0f}% chance of closing price be above {upper_close_value:.0f} on {next_day(date_of_data_point_x_used)}")
    else:
        assert False
