...
from utils import get_filename_for_dataset, DATASET_AVAILABLE, str2bool, get_month_weekday_range
import copy
import numpy as np
from datetime import datetime, timedelta
from crusaders.mmi.mmi_next import main as MMI_next
from crusaders.mmi.mmi_next_month_at_4_0p import CONFIGURATION_FOR_MMI_NEXT_MONTH, BEST_SCORE
...
def main(args):
    ...
    sensitivities = np.arange(1.012, 0.9900, -0.0012)

    date_prediction_step = MMI_next(Namespace(**config_dict))["date_prediction_step"]

    # Create Table
    tt1, tt2 = get_month_weekday_range(date_prediction_step)
    table = Table(title=f"4% Sensitivity {args.ticker} for month ({tt1}:{tt2})", box=box.ROUNDED, header_style="bold cyan")
