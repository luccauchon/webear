...
from config.default.train import *
###############################################################################


def main(cc):
    np.random.seed(cc.tr__seed_offset)
    torch.manual_seed(cc.tr__seed_offset)
    torch.cuda.manual_seed_all(cc.tr__seed_offset)
    torch.backends.cuda.matmul.allow_tf32 = True  # allow tf32 on matmul
    torch.backends.cudnn.allow_tf32 = True  # allow tf32 on cudnn

    os.makedirs(cc.tr__output_dir, exist_ok=True)
    logger.remove()
    logger.add(sys.stdout, level=cc.tr__debug_level)
    logger.add(os.path.join(cc.tr__output_dir, "train.txt"), level='DEBUG')
