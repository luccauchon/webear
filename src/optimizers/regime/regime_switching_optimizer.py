...
def run_real_time_inference(args, ticker, list_models, model_filename, use_enhanced_features):
    ...
    print(f" 🎯 Short Strike Distance:    {_metadata['strike_distance'] * 100:.1f}% from current price ({latest_close_value:.0f},{latest_date.strftime('%Y-%m-%d')}) , {_metadata['spread_type'].upper()} forward {regime_stats['forward_days']} {regime_stats['dataset_id']} "
