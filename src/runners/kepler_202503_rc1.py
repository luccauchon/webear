...
def start_runner(configuration):
    ...
    device                       = "cuda"
    _power_of_noise              = configuration.get("power_of_noise", 0.1)
    ...
    logger.info(f"Created meta model with {len(candidats)} models , inferencing [{start_date.date()}] to [{end_date.date()}]")
    # Iterate over days
